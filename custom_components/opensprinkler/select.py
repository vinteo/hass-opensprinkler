...
def _create_entities(hass: HomeAssistant, entry: dict):
    ...
    for _, program in controller.programs.items():
        entities.append(ProgramRestrictionSelect(entry, name, program, coordinator))

    return entities


class ProgramRestrictionSelect(
    OpenSprinklerProgramEntity, OpenSprinklerSelect, SelectEntity
):
    """Represent select for the odd/even restriction of a program."""

    def __init__(self, entry, name, program, coordinator):
        """Set up a new OpenSprinkler program select."""
